...
import type { JobSummary, SearchResult as ApiSearchResult, StashDBSceneCandidate } from '../api/client';
...
// ---------------------------------------------------------------------------
// StashDB lookup panel
// ---------------------------------------------------------------------------

interface StashDBLookupPanelProps {
  candidates: StashDBSceneCandidate[];
  loading: boolean;
  queueStatus: Record<string, { status: string; jobId?: string } | 'error'>;
  safeMode: boolean;
  onQueue: (sceneId: string) => void;
  onClose: () => void;
}

function StashDBLookupPanel({ candidates, loading, queueStatus, safeMode, onQueue, onClose }: StashDBLookupPanelProps) {
  return (
      ...
      {!loading && candidates.length > 0 && (
        <div className="flex flex-col gap-2">
          {candidates.map((scene) => {
            ...
            const label = queued
              ? (qs as { status: string }).status === 'already_stashed' ? 'Already in Stash' : '✓ Queued'
...
function DetailPanel({ jobId, onApproved, onSkipped, onListRefresh }: DetailPanelProps) {
  ...
  const [queueStatus, setQueueStatus] = useState<Record<string, { status: string; jobId?: string } | 'error'>>({});
