...
// Returns an array of segments — either plain text or {token} replacements.
// Unknown tokens are flagged so we can highlight them in the preview.
...
function parseTemplate(template: string): PreviewSegment[] {
  ...
  while ((match = regex.exec(template)) !== null) {
    ...
    const fullToken = match[0]; // e.g. "{studio}"
    const key = match[1];       // e.g. "studio"
...
function validateTemplate(template: string): ValidationResult {
  ...
  // Last segment (filename) must contain {ext}
  ...
  // At least one non-token character in the last segment
...
function renderPreview(template: string): React.ReactNode {
  if (!template.trim()) {
    return <span className="text-zinc-500 italic">Enter a template to see preview</span>;
  }
...
export default function TemplateBuilder() {
  ...
  // Initialise textarea from loaded config
  useEffect(() => {
    if (configData) {
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      <h1 className="text-xl font-semibold text-white mb-6">Directory Template Builder</h1>

      {configLoading && (
        <p className="text-zinc-400 mb-4">Loading current template…</p>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className="bg-zinc-900 border border-zinc-700 rounded-lg p-4 flex flex-col gap-3">
            <label htmlFor="template-input" className="text-sm font-medium text-zinc-300">
              Template
            </label>
            <textarea
              id="template-input"
              ...
              className="w-full bg-zinc-800 border border-zinc-600 rounded px-3 py-2 text-sm text-white font-mono placeholder-zinc-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 resize-y"
            />
          ...
          <div className="bg-zinc-900 border border-zinc-700 rounded-lg p-4 flex flex-col gap-3">
            <p className="text-sm font-medium text-zinc-300">
              Token Reference
              <span className="ml-2 text-xs text-zinc-500 font-normal">Click a token to insert it at cursor</span>
            </p>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-zinc-700">
                    <th className="text-left py-2 pr-4 text-zinc-400 font-medium whitespace-nowrap">Token</th>
                    <th className="text-left py-2 pr-4 text-zinc-400 font-medium">Description</th>
                    <th className="text-left py-2 text-zinc-400 font-medium whitespace-nowrap">Example</th>
                  </tr>
                </thead>
                <tbody>
                  {TOKENS.map((t) => (
                    <tr key={t.token} className="border-b border-zinc-800 hover:bg-zinc-800/50 transition-colors">
                      <td className="py-2 pr-4 whitespace-nowrap">
                        <button
                          onClick={() => insertToken(t.token)}
                          className="font-mono text-indigo-400 hover:text-indigo-300 hover:underline cursor-pointer focus:outline-none focus:ring-1 focus:ring-indigo-500 rounded px-0.5"
                          title={`Insert ${t.token}`}
                        >
                          {t.token}
                        </button>
                      </td>
                      <td className="py-2 pr-4 text-zinc-300">{t.description}</td>
                      <td className="py-2 text-zinc-400 font-mono text-xs whitespace-nowrap">{t.example}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        <div className="flex flex-col gap-4">
          <div className="bg-zinc-900 border border-zinc-700 rounded-lg p-4 flex flex-col gap-4">
            ...
            <div className="bg-zinc-800 rounded p-3 text-xs text-zinc-400 flex flex-col gap-1">
              <p className="text-zinc-500 font-medium mb-1 uppercase tracking-wide text-[11px]">Synthetic scene data</p>
              <div className="grid grid-cols-2 gap-x-4 gap-y-0.5">
                <span><span className="text-zinc-500">Studio:</span> {PREVIEW_DATA.studio}</span>
                <span><span className="text-zinc-500">Title:</span> {PREVIEW_DATA.title}</span>
                <span><span className="text-zinc-500">Performers:</span> {PREVIEW_DATA.performers}</span>
                <span><span className="text-zinc-500">Date:</span> {PREVIEW_DATA.date}</span>
                <span><span className="text-zinc-500">Resolution:</span> {PREVIEW_DATA.resolution}</span>
                <span><span className="text-zinc-500">Extension:</span> {PREVIEW_DATA.ext}</span>
                <span><span className="text-zinc-500">Duration:</span> {PREVIEW_DATA.duration}</span>
              </div>
            </div>

            {/* Rendered path */}
            <div>
              <p className="text-xs text-zinc-500 mb-1 uppercase tracking-wide font-medium">Rendered path</p>
              <div className="bg-zinc-800 rounded px-3 py-2 font-mono text-sm text-zinc-100 break-all leading-relaxed min-h-[2.5rem]">
            ...
            {renderedString && (
              <div>
                <p className="text-xs text-zinc-500 mb-1 uppercase tracking-wide font-medium">Filename segment</p>
                <div className="bg-zinc-800 rounded px-3 py-2 font-mono text-sm text-zinc-300 break-all">
                  {filenameSegment || <span className="text-zinc-500 italic">—</span>}
                </div>
                <div className={`mt-1.5 text-xs flex items-center gap-1 ${charWarning ? 'text-amber-400' : 'text-zinc-500'}`}>
            ...
            <div className="flex items-center gap-4 text-xs text-zinc-500 pt-1 border-t border-zinc-800">
              <span className="flex items-center gap-1">
                <span className="text-zinc-200 font-mono">text</span>
                <span>= substituted value</span>
              </span>
              <span className="flex items-center gap-1">
                <span className="text-red-400 font-mono font-semibold">{'{unknown}'}</span>
          ...
          {/* Example templates */}
          <div className="bg-zinc-900 border border-zinc-700 rounded-lg p-4 flex flex-col gap-3">
            <p className="text-sm font-medium text-zinc-300">Example Templates</p>
            <ul className="flex flex-col gap-2">
              {[
                ...
                <li key={ex.value}>
                  ...
                  >
                    <span className="block text-xs text-zinc-400 group-hover:text-zinc-200 transition-colors">{ex.label}</span>
                    <span className="block font-mono text-xs text-indigo-400 group-hover:text-indigo-300 transition-colors">{ex.value}</span>
                  </button>
                </li>
              ))}
            </ul>
